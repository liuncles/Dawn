using Dawn.Commons.HttpContextUser;
using Dawn.Serilog.Es;
    ...
    public class RecordAccessLogsMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            if (AppSettings.App("Middleware", "RecordAccessLogs", "Enabled").ObjToBool())
            {
                ...
                {
                    _stopwatch.Restart();
                    var userAccessEntity = new UserAccessEntity();

                    HttpRequest request = context.Request;

                    userAccessEntity.API = api;
                    userAccessEntity.User = _user.Name;
                    userAccessEntity.IP = IpLogMiddleware.GetClientIP(context);
                    userAccessEntity.BeginTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                    userAccessEntity.RequestMethod = request.Method;
                    userAccessEntity.Agent = request.Headers["User-Agent"].ObjToString();
                    ...
                    if (request.Method.ToLower().Equals("post") || request.Method.ToLower().Equals("put"))
                    {
                        ...
                        userAccessEntity.RequestData = Encoding.UTF8.GetString(buffer);

                        request.Body.Position = 0;
                    }
                    else if (request.Method.ToLower().Equals("get") || request.Method.ToLower().Equals("delete"))
                    {
                        userAccessEntity.RequestData = HttpUtility.UrlDecode(request.QueryString.ObjToString(), Encoding.UTF8);
                    }
                    ...
                    context.Response.OnCompleted(() =>
                    {
                        ...
                        userAccessEntity.OPTime = _stopwatch.ElapsedMilliseconds + "ms";

                        // 自定义log输出
                        var requestInfo = JsonConvert.SerializeObject(userAccessEntity);
                        //Parallel.For(0, 1, e =>
                        //{
