namespace Dawn.Commons.Seed
{
    public class ApplicationDbContext
    {
        ...
        {
            var mainConnetctDb = DBConfig.MutiConnectionString.allDbs.Find(x => x.ConnId == MainDb.CurrentDbConnId);
            if (DBConfig.MutiConnectionString.allDbs.Count > 0)
            {
                if (mainConnetctDb == null)
                {
                    mainConnetctDb = DBConfig.MutiConnectionString.allDbs[0];
                }
            }
            else
            {
