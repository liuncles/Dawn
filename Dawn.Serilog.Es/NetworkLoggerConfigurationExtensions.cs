using Dawn.Serilog.Es.Formatters;
using Dawn.Serilog.Es.Sinks.TCP;
using Serilog;
...
namespace Dawn.Serilog.Es
{
    ...
    public static class NetworkLoggerConfigurationExtensions
    {
        ...
        {
            // 读取相关配置
            var logConfigDtoInfo = JsonConfigUtils.GetAppSettings<LogConfigDto>(AppSettingsFileNameConfig.AppSettingsFileName, "LogFiedOutPutConfigs");
            if (logConfigDtoInfo == null)
            {
                return;
            }
            TcpAddressHost = logConfigDtoInfo.TcpAddressHost;
            TcpAddressProt = logConfigDtoInfo.TcpAddressPort;
        }
